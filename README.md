...
- **`lab3-heap/`** — сортування купою  
  - `HeapSort.java` — реалізація сортування купою 
- **`lab4-hash/`** — хеш-таблиці
  - `OpenHashTable.java` — відкрите хешування (ланцюжки)  
  - `ClosedHashTable.java` — закрите хешування (лінійне розв’язання колізій)
- **`lab5-graph/`** — мінімальні кістякові дерева (MST)
  - **Prim.java** — реалізація алгоритму Пріма  
  - **Kruskal.java** — реалізація алгоритму Крускала
